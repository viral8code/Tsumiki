...
namespace Tsumiki.Core.Output
{
    ...
    /// 「なぜそこで切れたか」の情報が contigs.fasta には残らない。
    /// GFA としてグラフそのものを出力すれば、Bandage 等のビューアで
    /// 「あと何が解ければ閉じるのか」を直接見られる。完全長を目指す作業は
    /// 本質的に反復的であり、診断可能性そのものが機能である。
    ///
    /// 出力するのは V_結合_コンティグ がバブル除去・反復解決を終えた後の
    /// グラフの状態(=最終的な walk がどの分岐を残したまま打ち切られたかを
    /// 反映する)。バブル除去前の生の de Bruijn グラフではない。
    /// </summary>
    internal static class GfaWriter
    {
        /// <summary>
        /// p_ユニティグ配列・p_グラフ の状態を GFA1 として p_パス へ書き出す。
        /// 頂点は unitig ID(1始まり)の順鎖/逆鎖のペアで表現されているため、
        /// 各物理的な隣接は双子の辺として2回現れる。片方だけを1本のL行として出す。
        /// </summary>
            ...
            IReadOnlyDictionary<int, int>? p_コピー数 = null)
        {
            ...
            // 同じ物理的隣接が v→w と w^1→v^1 の双子として2回現れるため、
            // 片方を出したらもう片方は出さない。
