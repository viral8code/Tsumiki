...
namespace Tsumiki.Core.Polishing
{
    ...
    /// 通り抜けたエラー k-mer がそのまま残ることがある。リードそのものの
    /// 多数決は k-mer とは独立した証拠で、そこを最後に均す。
    ///
    /// 直すのは置換だけとする。ここの照合は ungapped なので挿入・欠失は
    /// そもそも検出できず、検出できないものを直そうとすれば配列を壊す。
    ///
    /// 併せて位置ごとの深度が得られる。連結の裏付けが無い接合点はその前後で
    /// 深度が不連続になるため、完全長の判定にも使う。
    /// </summary>
    internal static class Polisher
    {
        /// <summary>
        /// リードの置き場所を探す種にする長さ。短いほど反復配列で曖昧になり、
        /// 長いほどエラーを1つ含んだだけで種が潰れる。
        /// </summary>
        private const int シード長 = 31;

        /// <summary>
        /// 参照側で種を登録する間隔。リード長はこれよりはるかに長いため、
        /// 間引いてもリードのどこかは必ず登録済みの位置に重なる。
        /// 全位置を持つと索引がゲノムサイズそのものの規模になる。
        /// </summary>
        private const int シード間隔 = 8;

        /// <summary>
        /// 1本のリードにつき試す種ヒットの数。反復配列では種が当たっても
        ...
        /// 磨く対象が無い(配列が空、種が1つも取れない)場合は null を返す。
        /// </summary>
            ...
            string p_FASTAパス, string p_リード1のパス, string? p_リード2のパス, string p_出力パス)
        {
            ...
            // 位置ごとの塩基の得票。塩基IDは 1..4 なので (位置 * 4 + 塩基ID - 1)。
        ...
        /// <summary>
        /// 参照配列から種索引を作る。順鎖と逆相補を別のキーで登録し、
        /// リードがどちらの向きで載ったかを引けるようにする。
        ...
        /// <summary>
        /// 1本のリードを置ける場所へ置き、各位置の得票を加算する。
        /// 置けたら true。
        ///
        /// 得票は複数のワーカーが同じ配列を触るため Interlocked で足す。
        ...
        /// <summary>
        /// リードが載っている位置での深度の中央値。ヒストグラムから求めるのは、
        /// 位置ごとの値をすべて並べるとゲノムサイズぶんの配列をもう1本
        /// 持つことになるため。
        ///
        /// 深度0の位置を混ぜないのが要点。混ぜると、覆われていない範囲が
        /// 広いアセンブリほど中央値が0へ引き寄せられ、本来そこを咎めるはずの
        /// 深度不足の判定が何も引っ掛けなくなる。
        /// </summary>
        ...
        /// <summary>
        /// 得票の多数決で置換を適用する。併せて深度不足の位置を数える。
        /// N の位置は触らない。ギャップの長さは推定値であり、
