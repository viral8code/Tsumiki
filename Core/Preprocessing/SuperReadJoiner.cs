...
namespace Tsumiki.Core.Preprocessing
{
    ...
    internal static class SuperReadJoiner
    {
        ...
        /// <summary>
        /// 1ペア分の統合を、どちらの手段で繋いだかと一緒に返す。
        ///
        /// 先にペアの重なりを試すのは、断片長がリード長の2倍を下回るライブラリでは
        /// read1 と RC(read2) が重なり、橋渡しに必要な長さが負になるため。
        ...
        /// <summary>
        /// read1 と RC(read2) の重なりから断片を復元する。重なりが見つからない
            ...
            ref int p_曖昧で捨てた数)
        {
            ...
            // 断片が read1 に収まっているならアダプタ読み抜けで、前処理の
            // トリムの領分。ここで作っても長さが伸びない。
        ...
        ///
        /// 重なりに許容範囲内の不一致が残っていると、繋いだ配列にはどちらの
        /// リードとも違う塩基が入り、継ぎ目を跨ぐ k-mer だけがどのリードにも
        /// 存在しないものになる。存在しない配列をグラフへ持ち込まないための関門。
        /// (重なりが完全一致なら継ぎ目の k-mer は read2 側の k-mer と一致するため、
        ///  反復配列の別コピーを掴む取り違えはここでは防げない。それは
        ///  最小重なり長で抑える。)
        /// </summary>
        private static bool Get_継ぎ目が支持されているか(
            string p_合成, int p_継ぎ目, TrustedKmerIndex p_kmerインデックス, int p_k長)
        {
        ...
        /// <summary>
        /// read1 の末尾 k-mer から RC(read2) の先頭 k-mer まで、信頼できる
        /// k-mer 集合の中で経路を探して繋ぐ。どちらかの端の k-mer が集合に無い、
        /// あるいは経路が一意に定まらない場合は null。
        /// </summary>
        private static string? Get_橋渡しで結合(
            string p_配列1, string p_配列2, TrustedKmerIndex p_kmerインデックス, int p_k長,
            int? p_インサートサイズ)
        {
