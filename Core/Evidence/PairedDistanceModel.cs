...
namespace Tsumiki.Core
{
    ...
    /// 観測されるという幾何的な偏りをそのまま拾う。期待本数が数本しかない場所と
    ...
    internal sealed class PairedDistanceModel
    {
        ...
        /// ペアが長い裾を作るため。裾が過半を占めても、同じ隣接から出た
        /// ペアは狭い範囲に集まるので峰は残る。
        /// </summary>
        ...
        /// フラグメントの開始位置の総数。フラグメント開始位置の密度を掛けると
        /// 期待ペア数になる。
        ///
        /// 接合点を原点とし、フラグメント長 x の開始位置 s について
        /// 左リードが左側に収まる条件 s &lt;= -リード長 かつ s &gt;= -長さ1、
        /// 右リードが右側に収まる条件 s &gt;= ギャップ長 + リード長 - x かつ
        /// s &lt;= ギャップ長 + 長さ2 - x を満たす s の個数を数える。
        /// </summary>
