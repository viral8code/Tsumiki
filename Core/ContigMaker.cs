...
namespace Tsumiki.Core
{
    internal class ContigMaker
    {
        ...
        ///
        /// 2ヒットの順鎖座標の差はフラグメント長ではなく、2リードに挟まれた
        /// 内側の未読区間(inner distance)である。FR配置では
        ...
        /// <summary>
        /// 標本抽出された距離の一覧。Scaffolder は出所によるバイアスの違いを
            ...
            string? p_GFAパス = null)
        {
            ...
            // 1本のリードでは跨げない長さの反復も、フラグメント長なら跨げる。
            ...
            for (var v = 2; v < l_グラフ.A_出辺.Count; v++)
            {
                ...
                // 各コピーの続きが1頂点に集まるため支持が全ての行き先に付き、
                // 行き先を選ぶ根拠にならない。正しい続きは反復の外側の
            ...
            for (var c = 0; c < l_コンティグ群.Count; c++)
            {
                ...
                if (l_環状フラグ群[c])
                {
                    ...
                    // 産物でしかない)。下流の比較・再現性のため、辞書式順序で
                    // 最小になる回転へ正規化する(鎖の向きは上の比較で既に
                    // 決めているため、ここでは回転のみ。鎖の選択まで変えると
        ...
        /// <summary>
        /// フラグメント長分布の分位点。橋渡しできる未知区間の長さを決めるのは
