...
CLI は standard 設定が既定です。`Tsumiki.exe -1 reads.1.fq -2 reads.2.fq -t run` だけで前処理・訂正・multi-k・SuperRead・反復検証・局所充填・GFA・polish・環状検証・mercy 救済を有効にします。コピー数基準は weighted、端トリミングは有効、結果マージは無効です。従来の opt-in 設定は `-profile legacy` で選べます。プロファイル指定は個別の上書き（例: `-cnb spectrum`、`-nt`、`-mg`）より前に置いてください。`-k 49` は単独 k を指定します。最終 k では次段用の SuperRead を生成しません。weighted は運用上の既定変更であり、全量データでの精度改善を保証するものではありません。
...
| `-pp` | アダプター除去 & リード間オーバーラップ訂正 | On |
| `-ec` | k-mer スペクトラムを用いたリードエラー訂正 | On |
| `-mk` | 複数 k 候補の生成・評価を有効化 | On |
| `-nc` | k 実行間での配列引き継ぎ（Carry-over）を無効化 | 有効 (Off 指定で無効) |
| `-sr` | オーバーラップペアからの合成配列引き継ぎ | On |
| `-mg` | 異言語 / 別 k アセンブリ結果の統合・マージ | Off |
| `-rv` | リピート接合部での厳密な r-mer エビデンスを要求 | On |
| `-la` | 局所ギャップアセンブリ (曖昧領域の長 k 再試行) | On |
| `-my` | 低頻度 k-mer の救出処理 | On |
| `-po` | 最終アセンブリの置換ポリッシング (修復) | On |
| `-cc` | 環状接合部を跨ぐリードの整合性検証 | On |
| `-gfa` | GFA1 形式での Unitig グラフ出力 | On |
