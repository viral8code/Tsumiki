...
namespace Tsumiki.Utility
{
    ...
    internal static class KmerSpectrumMixtureModel
    {
        ...
        /// <summary>
        /// コピー数別の混合比 π_k は自由な10パラメータにせず、
        /// π_k ∝ r^(k-1) という単一の減衰率 r で表す(k=1が最大、以降単調減少)。
        ///
        /// 自由な10パラメータのままだと、λ' = λ/d (d は2以上の約数) にして
        /// k'=d, 2d, 3d... にだけ重みを乗せれば、真のピーク位置(dλ', 2dλ', ...)を
        /// そっくりそのまま再現できてしまう「調波エイリアシング」の別解にEMが
        /// 収束しうる。単調減少という1パラメータの制約は、真のゲノムで高コピー
        ...
        /// <summary>
        /// 誤り成分の初期平均。出現回数1..20の頻度加重平均を使う
        /// (低頻度域の大まかな水準を見るだけの粗い初期値で、EMが精密化する)。
        /// 定数1で初期化すると、幾何分布が c=1 にのみ質量を持つ退化点になり、
        ...
        /// <summary>
        /// EM の初期値候補。頻度の高い局所極大(データが実際に示す山)と、
        /// 対数間隔のグリッド(局所極大が谷に埋もれて見えない場合の保険)を併用する。
        /// 谷の目視判定には依存しない。
        /// </summary>
        ...
        /// <summary>
        /// E-step。各出現回数について、誤り成分・コピー数1..上限の各成分への
        ...
        /// <summary>
        /// M-step。責任度で重み付けした最尤推定でパラメータを更新する。
        /// どちらかの成分が完全に空(重みの総和が0)になった場合は false を返す。
        ///
        /// r(コピー数の減衰率)は、コピー数の重み付き平均 m から
        /// (幾何分布の平均 = 1/(1-r) の関係を使って) r = 1 - 1/m として更新する。
        /// 打ち切り(コピー数上限10)の影響を無視した近似だが、r が極端に1に
