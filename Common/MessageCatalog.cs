...
namespace Tsumiki.Common
{
    ...
    internal static class MessageCatalog
    {
        ...
        private static readonly Dictionary<メッセージID, string> _英語 = new()
        {
            ...
            [メッセージID.SuperRead統計_曖昧] =
                "[SuperRead] {0} pair(s) had more than one plausible overlap and were left to the graph search (a repeat would fit at several offsets).",
            [メッセージID.短い配列を除外] =
            ...
            [メッセージID.最終成果物] =
                "[Info] Final assembly: {0} -- unitigs/contigs/scaffolds are the stages before merging, short-sequence removal and polishing.",
            [メッセージID.ギャップ充填_対象なし] =
        ...
        private static readonly Dictionary<メッセージID, string> _日本語 = new()
        {
            ...
            [メッセージID.先読みで解決した分岐数] =
                "[Debug] 先読み探索により、1手先の相互一意判定では決められなかった接合点をさらに {0} 箇所解決",
            [メッセージID.GFA出力完了] =
                "[Info] ユニティググラフを {0} に書き出し (GFA1)",
            [メッセージID.分岐選択の重み内訳] =
                "[Debug] 分岐選択の重み: 単一リード由来の隣接 {0} 組 + ペアエンド由来 {1} 組",
            [メッセージID.単純化の収束] =
                "[Debug] 単純化は {0} ラウンドで収束",
            [メッセージID.単純化の打ち切り] =
                "[Debug] 単純化はラウンド上限 ({0}) に達し、収束しきらずに打ち切り",
            [メッセージID.バブル除去数] =
                "[Debug] 単純バブルの枝を合計 {0} 本除去 (配列自体は独立したコンティグとして残し、グラフ上の辺だけ除外)",
            [メッセージID.反復解決数] =
                "[Debug] 反復解決: 短い反復 (<= {1}bp) を合計 {0} 箇所、跨いだリードペアを使って複製・分離",
            [メッセージID.辺選択の内訳] =
                "[Debug] 辺の選択: 出辺が1本だけの頂点 {0} 個、リード支持で解決した分岐 {1} 個、多コピー反復から出るため未解決の分岐 {2} 個 (反復内部のリードではコピーを区別できない)",
            [メッセージID.反復通り抜けで棄却した結合数] =
                "[Debug] 未分離の多コピー反復を通り抜ける形になるため、{0} 件の結合を見送り (通すと反復のコピー間にある配列を飛ばしてしまう)",
            [メッセージID.相互一意で残った結合数] =
                "[Debug] 相互一意の検査を通った有向マージ: {0} 件 (無向では {1} 件)",
            [メッセージID.環状コンティグあり] =
                "[Info] 環状に閉じたコンティグ: {0} 本 ({1}) -- そのレプリコン (染色体またはプラスミド) が端から端まで組み上がったことを示す",
            [メッセージID.環状コンティグなし] =
                "[Info] 環状に閉じたコンティグなし -- どのレプリコンも断片のまま",
            [メッセージID.確定辺標本数] =
                "[Info] 確定した (実際に結合された) ユニティグ隣接から得たインサートサイズ標本: {0} 件",
            [メッセージID.確定辺標本の中央値] =
                "[Info] 確定辺標本の中央値: {0} ({1} 件の標本、同一ユニティグ標本のような長さの偏りを受けない)",
            [メッセージID.短すぎるユニティグの除外] =
                "[Warning] k-mer 長より短いユニティグ {0} 本をマッピング対象から除外",
            [メッセージID.曖昧なkmer登録] =
                "[Warning] 曖昧な (複数のユニティグで共有された) k-mer 登録 {0} 件をマッピングでは無視",
            [メッセージID.ペア隣接候補数] =
                "[Info] ペアエンド由来の隣接候補: 辺 {0} 本 (支持したペアは延べ {1} 組)",
            [メッセージID.同一ユニティグのペア向き集計] =
                "[Info] 同一ユニティグ内のペアの向き: 同方向={0}、逆方向={1} -- インサートサイズ推定にはこのライブラリの向きとして '{2}' を採用 ({3} 件の標本)",
            [メッセージID.同一ユニティグの断片長分布] =
                "[Info] 同一ユニティグ内の断片長分布: {0}",
            [メッセージID.同一ユニティグの断片長中央値] =
                "[Info] 同一ユニティグ内の断片長中央値: {0} ({1} 件の標本、内側の距離にリード長を戻した真の断片長、ただしユニティグが真のインサートサイズより短い場合は短めに偏りうる)",
            [メッセージID.ペアリードIDの不一致] =
                "[Warning] この位置でペアのリードIDが不一致 (\"{0}\" と \"{1}\") -- 以降のリードではペアエンド由来の隣接検出が当てにならない可能性あり (単一リード由来の隣接検出には影響しない)",
            [メッセージID.統合できる接合点なし] =
                "[Merge] 骨格の接合点を跨げた k なし -- 骨格をそのまま使用",
            [メッセージID.統合した接合点数] =
                "[Merge] 他の k の配列で骨格の接合点 {0} 箇所を連結",
            [メッセージID.候補一覧の見出し] =
            ...
            [メッセージID.検査_対象外のk長] =
                "[Check] {0}: スキップ (自己検査は k <= 64 のみ対応)",
            [メッセージID.検査_取りこぼし] =
                "[Check] {0}: 信頼できる k-mer {1:N0} 個のうち {2:N0} 個 ({3:0.00}%) がアセンブリに現れない (削られたか、どの経路も通らなかった配列)",
            [メッセージID.検査_出しすぎ] =
                "[Check] {0}: アセンブリ内の k-mer は延べ {1:N0} 個、うち {2:N0} 個 ({3:0.00}%) がカバレッジの許すコピー数を超過 ({4:N0} 種類の k-mer) -- 総延長が水増しになっている部分",
            [メッセージID.kmerインデックス構築開始] =
                "k-mer インデックスの構築を開始",
            [メッセージID.kmerカットオフ適用] =
                "k-mer カットオフを適用",
            [メッセージID.引き継ぎで追加したkmer数] =
                "[Carry-over] 前の k から、この k では観測されなかった k-mer を {0:N0} 個追加",
            [メッセージID.tip除去開始] =
                "短い tip を除去",
            [メッセージID.ユニティグ構築開始] =
                "ユニティグを構築",
            [メッセージID.グラフが複雑すぎる] =
                "[Warning] k={0} ではグラフが複雑すぎてアセンブリ不可 (ユニティグ数が {1} を超過) -- この k をスキップ",
            [メッセージID.リードのマッピング開始] =
                "リードをユニティグへマッピング",
            [メッセージID.ユニティグ結合開始] =
                "ユニティグを結合",
            [メッセージID.rMer検証の見送り] =
                "[Info] k={0} では反復の r-mer 検証をスキップ -- r-mer 長 {1}bp が 32bp のパック上限を超えるため",
            [メッセージID.コンティグ構築完了] =
                "コンティグの構築完了",
            [メッセージID.スキャフォールディング開始] =
                "コンティグをスキャフォールディング",
            [メッセージID.ギャップ充填開始] =
                "スキャフォールドのギャップを充填",
            [メッセージID.kが薄すぎて省略] =
                "[Multi-k] k={0} をスキップ -- 予測 k-mer カバレッジ {1:F1}x が {2:F1}x を下回り、使えるグラフにならない",
            [メッセージID.kの開始見出し] =
                "[Multi-k] ===== k={0} でアセンブリ =====",
            [メッセージID.kでアセンブリできず] =
                "[Multi-k] k={0} ではアセンブリできず -- この k をスキップ",
            [メッセージID.単一のkのみ成功] =
                "[Multi-k] アセンブリできた k は1つのみ -- 比較せずに採用",
            [メッセージID.アンカーkmer集合の構築] =
                "[Multi-k] 比較用の共通アンカー k-mer 集合 (k={0}) を構築",
            [メッセージID.アンカースペクトルが二峰でない] =
                "[Multi-k] アンカーの k-mer スペクトルが二峰でないため、候補を比較できず",
            [メッセージID.候補を評価できない] =
                "[Multi-k] 候補を評価できないため、最大の k を採用",
            [メッセージID.採用したk] =
                "[Multi-k] k={0} を採用",
            [メッセージID.統合開始] =
                "[Merge] 採用したアセンブリへ、他の k の配列を統合",
            [メッセージID.統合結果を評価できない] =
                "[Merge] 統合結果を評価できないため、採用済みのものを維持",
            [メッセージID.統合前の評価] =
                "[Merge]   統合前: {0}",
            [メッセージID.統合後の評価] =
                "[Merge]   統合後: {0}",
            [メッセージID.統合が骨格に勝てず] =
                "[Merge] 統合結果は採用済みのものに及ばず -- そのまま維持",
            [メッセージID.統合結果を採用] =
                "[Merge] 統合結果を採用",
            [メッセージID.エラー訂正_スペクトル構築] =
            ...
            [メッセージID.エラー訂正_ファイル別統計] =
                "[ErrorCorrection] {0}: {1}/{2} リードを訂正 (訂正塩基は延べ {3} 箇所)",
            [メッセージID.リード読込完了] =
                "{1} から {0} リードを読み込み",
            [メッセージID.リード2の読込開始] =
                "ファイル2を読み込み",
            ...
            [メッセージID.SuperRead統計_曖昧] =
                "[SuperRead] {0} 組は重なりの位置が一つに定まらずグラフ探索へ回した (反復配列では複数の位置が同じくらい良く合う)",
            [メッセージID.短い配列を除外] =
                "[Info] 最終成果物から {1}bp 未満の配列 {0} 本 (計 {2}bp) を除外 -- k ごとの成果物には残る",
            [メッセージID.最終成果物] =
                "[Info] 最終成果物: {0} -- unitigs/contigs/scaffolds は統合・短い配列の除外・ポリッシュより前の各段階",
            [メッセージID.ギャップ充填_対象なし] =
                "[Info] ギャップ充填: 埋めるギャップなし",
            [メッセージID.ギャップ充填統計] =
                "[Info] ギャップ充填: {1} 箇所のうち {0} 箇所を実配列で充填 ({2:N0}bp の N を置換)、{3} 箇所は経路が複数あるため、{4} 箇所は両側を繋ぐ経路がグラフ上に無いため N のまま",
            [メッセージID.局所アセンブリ_対象なし] =
                "[Info] 局所アセンブリ: 対象となる残りギャップなし",
            [メッセージID.局所アセンブリ統計] =
                "[Info] 局所アセンブリ: 残り {1} 箇所のうち {0} 箇所を局所的に組み直したリードで充填 ({2:N0}bp の N を置換)、{3} 箇所は局所リードなし、{4} 箇所は局所経路が複数、{5} 箇所は両側を繋ぐ局所経路なし",
            [メッセージID.スキャフォールディング省略_インサートサイズ不明] =
                "[Info] スキャフォールディングをスキップ -- インサートサイズが未指定で、マップされたペアからも推定できず",
            [メッセージID.スキャフォールディング開始_インサートサイズ] =
                "[Info] インサートサイズ {0} でスキャフォールディング",
            [メッセージID.スキャフォールディング省略_コンティグなし] =
                "[Info] スキャフォールディングをスキップ -- コンティグなし",
            [メッセージID.内部を指したペア候補] =
                "[Info] 結合済みのコンティグ内部にあるユニティグを指すペアエンド候補 {0} 件をスキップ (端点はコンティグ構築の時点で解決済み)",
            [メッセージID.未配置を指したペア候補] =
                "[Info] どのコンティグにも配置されなかったユニティグ (短すぎる等) を指すペアエンド候補 {0} 件をスキップ",
            [メッセージID.閾値後のスキャフォールド辺] =
                "[Info] 閾値処理後に確定したスキャフォールド辺: {0} 本 (相互一意の検査で {1} 本を棄却、{2} 本を採用)",
            [メッセージID.スキャフォールド出力完了] =
                "[Info] スキャフォールド {0} 本 (総延長 {1}) を {2} に書き出し",
            [メッセージID.インサートサイズ推定_同一ユニティグ] =
                "[Info] インサートサイズを {1} 組の同一ユニティグ標本から {0} と推定 (中央値、ユニティグ N50 {2} が推定値の {3} 倍以上のため短い断片への打ち切りバイアスは働かない)",
            [メッセージID.インサートサイズ推定_確定辺] =
                "[Info] インサートサイズを {1} 組の確定辺標本から {0} と推定 (中央値、ユニティグが十分に長くなく同一ユニティグ標本は偏るためこちらを優先)",
            [メッセージID.インサートサイズ推定_標本不足] =
                "[Info] インサートサイズの自動推定には最低 {0} 件の標本が必要、確定辺由来 {1} 件、全体でも {2} 件のみ",
            [メッセージID.インサートサイズ推定_全標本] =
                "[Info] インサートサイズを {1} 組の標本から {0} と推定 (中央値、確定辺由来の標本が {2} 件と少ないため全標本に回帰、短めに偏る可能性あり)",
            [メッセージID.単一コピー基準値] =
                "[Info] 単一コピーのカバレッジ基準値: {0:0.#} (長さで重み付けした中央値)",
            [メッセージID.コピー数の要約] =
                "[Info] 推定コピー数 -- {0}",
            [メッセージID.反復配列の割合] =
                "[Info] 多コピー (反復) の量: {1:N0}bp 中 {0:N0}bp (アセンブリの {2:0.0}% が2回以上現れる配列)",
            [メッセージID.グラフ単純化の反復] =
                "[GraphSimplifier] 反復 {0}: ユニティグ {1} 本を検査 (tip 閾値 {2}bp 未満、カバレッジ基準値 {3:0.#})、tip を {4} 本除去、{6} 本のユニティグ端から低カバレッジの k-mer を {5} 個除去",
            [メッセージID.rMer検証による棄却] =
                "[Debug] r-mer 検証により、接合点を跨ぐリードで裏付けの取れない反復の複製 {0} 件を棄却 (ペア数の証拠だけなら複製されていたもの)",
            [メッセージID.例外を無視_見出し] =
                "[Warning] 以下の例外は無視して続行",
            [メッセージID.例外を無視_メソッド] =
                "          メソッド: {0}",
            [メッセージID.停止_見出し] =
                "[Error] 処理を中断",
            ...
            [メッセージID.タイムスタンプ] =
                "[Log] {0:yyyy/MM/dd HH:mm:ss}",
            [メッセージID.Phred_明示指定と不一致] =
                "[Warning] クオリティ列は Phred{0} に見えるが、-p {1} が明示指定されているため指定値を優先 -- 実際に Phred{3} であれば -p {2} で再実行してください",
            [メッセージID.Phred_自動判定] =
                "[Info] クオリティ列から Phred オフセットを {0} と自動判定 (観測した ASCII 範囲 [{1}, {2}]) -- -p の明示指定で上書き可能",
            [メッセージID.Phred_検査の警告] =
                "[Warning] {0} の Phred エンコーディング検査: {1}",
            [メッセージID.kmerカットオフ_混合モデル] =
                "[Info] k-mer カットオフを2成分混合モデル (エラー + ゲノム) から {0} と自動選択 (単一コピー平均 {1:0.#}、信頼下限 {2}、EM 反復 {3} 回) -- -kc の明示指定で上書き可能",
            [メッセージID.kmerカットオフ_谷が不明] =
                "[Info] ヒストグラムの谷を特定できず (このカバレッジではスペクトルが二峰でない可能性あり) -- -kc {0} を維持",
            [メッセージID.kmerカットオフ_スペクトル] =
                "[Info] k-mer カットオフを k-mer スペクトルから {0} と自動選択 -- -kc の明示指定で上書き可能",
            [メッセージID.kmerヒストグラム] =
                "[Info] k-mer 出現回数のヒストグラム (出現回数:異なる k-mer 数): {0}",
            [メッセージID.スペクトルの谷が不明] =
                "[Info] ヒストグラムの谷を特定できず (このカバレッジではスペクトルが二峰でない可能性あり)",
            [メッセージID.スペクトルの谷とピーク] =
                "[Info] k-mer スペクトル: 谷は出現回数 {0} ({1} 種類)、単一コピーのピークは出現回数 {2} ({3} 種類)",
            [メッセージID.ゲノムサイズとカバレッジの推定] =
                "[Info] 推定ゲノムサイズ: {0:N0} bp、推定カバレッジ: {1}",
            [メッセージID.k自動選択_リード長不明] =
                "[Info] リード長を標本抽出できず -- -k {0} を維持",
            [メッセージID.k自動選択_kが長すぎる] =
                "[Warning] -k {0} は観測されたリード長 ({1} bp) より短くないため、リードから k-mer を抽出できません -- -k を小さくしてください",
            [メッセージID.k自動選択_リード長が短い] =
                "[Info] 観測されたリード長 ({0} bp) は k の自動選択には短すぎ -- -k {1} を維持",
            [メッセージID.k自動選択] =
            ...
            [メッセージID.パスの確認] =
                "パスを確認してください",
            [メッセージID.前処理省略_ペアなし] =
                "[Warning] -pp はペアエンドのリードが必要 (read2 が未指定) -- 前処理をスキップ",
            [メッセージID.前処理開始] =
            ...
            [メッセージID.理想本数モデルなし] =
                " -- 理想本数モデルは利用不可",
            [メッセージID.Phred_ファイル間で不一致] =
                "[Warning] 2つのリードファイルで Phred オフセットの推定が不一致 (read1 -> {0}、read2 -> {1}) -- -p {2} をそのまま使用",
            [メッセージID.Phred_未確定] =
                "不明",
            [メッセージID.kmer種類数] =
                "[Debug] k-mer の種類数: {0}",
            ...
            [メッセージID.引き継ぎの準備開始] =
                "次の k へ引き継ぐ配列を準備中",
            [メッセージID.引き継ぎの準備完了] =
                "[Carry-over] 次の k へ引き継ぐ配列を {0:N0} 本用意",
            [メッセージID.SuperRead橋渡し開始] =
        ...
        private static readonly Dictionary<メッセージID, string> _中国語 = new()
        {
            ...
            [メッセージID.先読みで解決した分岐数] =
                "[Debug] 束搜索前瞻额外解决单步互唯一性判定无法决定的连接点 {0} 处",
            [メッセージID.GFA出力完了] =
            ...
            [メッセージID.バブル除去数] =
                "[Debug] 共移除简单气泡分支 {0} 条（序列本身作为独立 contig 保留，仅移除图中的边）",
            [メッセージID.反復解決数] =
                "[Debug] 重复解析：短重复（<= {1}bp）共 {0} 处，借助跨越它们的 read 对完成复制与解缠",
            [メッセージID.辺選択の内訳] =
                "[Debug] 边选择：出边唯一的顶点 {0} 个，凭 read 支持解决的分支 {1} 个，因离开多拷贝重复而未解决的分支 {2} 个（重复内部的 read 无法区分各拷贝）",
            [メッセージID.反復通り抜けで棄却した結合数] =
                "[Debug] 因会穿过尚未解缠的多拷贝重复，放弃连接 {0} 处（穿过会跳过重复各拷贝之间的序列）",
            [メッセージID.相互一意で残った結合数] =
                "[Debug] 通过互唯一性检查的有向合并：{0} 处（无向 {1} 处）",
            [メッセージID.環状コンティグあり] =
                "[Info] 闭合成环的 contig：{0} 条（{1}）-- 表示该复制子（染色体或质粒）已被完整组装",
            [メッセージID.環状コンティグなし] =
            ...
            [メッセージID.短すぎるユニティグの除外] =
                "[Warning] 短于 k-mer 长度的 unitig {0} 条，已排除在比对之外",
            [メッセージID.曖昧なkmer登録] =
                "[Warning] 存在歧义（被多条 unitig 共享）的 k-mer 登记 {0} 处，比对时忽略",
            [メッセージID.ペア隣接候補数] =
                "[Info] 双端相邻候选：{0} 条边（共 {1} 对支持的 read 对）",
            [メッセージID.同一ユニティグのペア向き集計] =
                "[Info] 同一 unitig 内 read 对的方向：同向={0}，反向={1} -- 插入片段长度估计采用该文库观测到的方向 '{2}'（{3} 个样本）",
            [メッセージID.同一ユニティグの断片長分布] =
                "[Info] 同一 unitig 内片段长度分布：{0}",
            [メッセージID.同一ユニティグの断片長中央値] =
                "[Info] 同一 unitig 内片段长度中位数：{0}（共 {1} 个样本，已把 read 长度加回内部距离，是真实的片段长度，但 unitig 短于真实插入片段时仍可能偏短）",
            [メッセージID.ペアリードIDの不一致] =
                "[Warning] 此处配对 read 的 ID 不一致（\"{0}\" 与 \"{1}\"）-- 此后的 read 其双端相邻检测可能不可靠（单条 read 的相邻检测不受影响）",
            [メッセージID.統合できる接合点なし] =
                "[Merge] 没有其他 k 跨越骨架的连接点 -- 保持原样",
            [メッセージID.統合した接合点数] =
                "[Merge] 使用其他 k 的序列连接骨架的连接点 {0} 处",
            [メッセージID.候補一覧の見出し] =
            ...
            [メッセージID.検査_出しすぎ] =
                "[Check] {0}：组装中 k-mer 实例共 {1:N0} 个，其中 {2:N0} 个（{3:0.00}%）超出覆盖度所能支持的拷贝数（涉及 {4:N0} 种 k-mer）-- 即总长度虚增的部分",
            [メッセージID.kmerインデックス構築開始] =
                "开始构建 k-mer 索引",
            [メッセージID.kmerカットオフ適用] =
                "正在应用 k-mer 截断阈值",
            [メッセージID.引き継ぎで追加したkmer数] =
                "[Carry-over] 从上一个 k 补充当前 k 未观测到的 k-mer {0:N0} 个",
            [メッセージID.tip除去開始] =
                "正在修剪短 tip",
            [メッセージID.ユニティグ構築開始] =
                "正在构建 unitig",
            ...
            [メッセージID.kの開始見出し] =
                "[Multi-k] ===== 使用 k={0} 进行组装 =====",
            [メッセージID.kでアセンブリできず] =
                "[Multi-k] k={0} 未产出组装结果 -- 跳过该 k",
            [メッセージID.単一のkのみ成功] =
                "[Multi-k] 仅有一个 k 产出组装结果 -- 不作比较直接采用",
            [メッセージID.アンカーkmer集合の構築] =
            ...
            [メッセージID.エラー訂正_ファイル別統計] =
                "[ErrorCorrection] {0}：已校正 read {1}/{2} 条（共校正碱基 {3} 个）",
            [メッセージID.リード読込完了] =
                "已从 {1} 读入 read {0} 条",
            [メッセージID.リード2の読込開始] =
                "正在读取文件 2",
            ...
            [メッセージID.SuperRead統計_曖昧] =
                "[SuperRead] {0} 对的重叠位置无法唯一确定，交由图搜索处理（重复序列在多个位置都能对上）",
            [メッセージID.短い配列を除外] =
                "[Info] 从最终结果中剔除了 {0} 条短于 {1}bp 的序列 (共 {2}bp) -- 各 k 的结果中仍保留",
            [メッセージID.最終成果物] =
                "[Info] 最终结果：{0} —— unitigs/contigs/scaffolds 是合并、剔除短序列与打磨之前的各阶段输出",
            [メッセージID.ギャップ充填_対象なし] =
                "[Info] 空缺填补：没有需要填补的空缺",
            [メッセージID.ギャップ充填統計] =
                "[Info] 空缺填补：{1} 处中有 {0} 处用真实序列填补（替换 {2:N0}bp 的 N），{3} 处因存在多条路径、{4} 处因图中没有连接两侧的路径而保留为 N",
            [メッセージID.局所アセンブリ_対象なし] =
                "[Info] 局部组装：没有符合条件的剩余空缺",
            [メッセージID.局所アセンブリ統計] =
                "[Info] 局部组装：剩余 {1} 处中有 {0} 处用局部重组的 read 填补（替换 {2:N0}bp 的 N），{3} 处没有任何局部 read，{4} 处存在多条局部路径，{5} 处没有连接两侧的局部路径",
            [メッセージID.スキャフォールディング省略_インサートサイズ不明] =
            ...
            [メッセージID.内部を指したペア候補] =
                "[Info] 指向已连接 contig 内部 unitig 的双端候选 {0} 个，已跳过（端点在 contig 构建时已解决）",
            [メッセージID.未配置を指したペア候補] =
                "[Info] 指向未被放入任何 contig 的 unitig（如过短）的双端候选 {0} 个，已跳过",
            [メッセージID.閾値後のスキャフォールド辺] =
                "[Info] 阈值处理后确定的 scaffold 边：{0} 条（互唯一性检查拒绝 {1} 条，保留 {2} 条）",
            [メッセージID.スキャフォールド出力完了] =
                "[Info] 已将 scaffold {0} 条（总长 {1}）写入 {2}",
            [メッセージID.インサートサイズ推定_同一ユニティグ] =
                "[Info] 依据 {1} 对同一 unitig 样本，将插入片段长度估计为 {0}（中位数，unitig N50 为 {2}，达到估计值的 {3} 倍以上，故不受短片段截断偏差影响）",
            [メッセージID.インサートサイズ推定_確定辺] =
                "[Info] 依据 {1} 对已确定边样本，将插入片段长度估计为 {0}（中位数，因 unitig 长度不足会使同一 unitig 样本产生偏差，故优先采用）",
            [メッセージID.インサートサイズ推定_標本不足] =
                "[Info] 插入片段长度自动估计至少需要 {0} 个样本，但仅收集到已确定边样本 {1} 个、总计 {2} 个",
            [メッセージID.インサートサイズ推定_全標本] =
                "[Info] 依据 {1} 对样本，将插入片段长度估计为 {0}（中位数，已确定边样本仅 {2} 个过少，故回退到全部样本，可能偏短）",
            [メッセージID.単一コピー基準値] =
                "[Info] 单拷贝覆盖度基线：{0:0.#}（按长度加权的中位数）",
            [メッセージID.コピー数の要約] =
                "[Info] 估计的拷贝数 -- {0}",
            [メッセージID.反復配列の割合] =
            ...
            [メッセージID.rMer検証による棄却] =
                "[Debug] r-mer 验证否决重复复制 {0} 处，其胜出配对未被真正跨越连接点的 read 证实（仅凭 read 对计数证据本会复制）",
            [メッセージID.例外を無視_見出し] =
                "[Warning] 以下异常将被忽略",
            [メッセージID.例外を無視_メソッド] =
                "          方法：{0}",
            ...
            [メッセージID.引き継ぎの準備開始] =
                "正在准备传递给下一个 k 的序列",
            [メッセージID.引き継ぎの準備完了] =
                "[Carry-over] 已为下一个 k 准备序列 {0:N0} 条",
            [メッセージID.SuperRead橋渡し開始] =
