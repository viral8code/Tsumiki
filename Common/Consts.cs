namespace Tsumiki.Common
{
    internal class Consts
    {
        ...
        /// 下げるか。候補と同じ k にすると、その候補だけが自分と同じ条件で
        /// 作った k-mer 集合で測られ、物差しとして中立でなくなる。
        /// </summary>
        public const int アンカーk長の候補からの差 = 2;

        /// <summary>
        /// アンカー k の下限。これ以上小さくすると、細菌のゲノム規模でも
        ...
        /// <summary>
        /// r-mer のふるいに確保するビット数の上限 (2^31 bit = 256MB)。
        /// </summary>
        public const long rMerふるいのビット数上限 = 1L << 31;

        /// <summary>
        /// r-mer のふるいで1件あたりに立てるビットの数。
        /// </summary>
        public const int rMerふるいのハッシュ数 = 4;

        /// <summary>
        /// r-mer 検証を行うために、1本のリードから最低限取れてほしい窓の数。
        ///
        /// r がリード長に近づくと窓が数個しか取れず、r-mer のカバレッジが
        /// リードのカバレッジの数%まで落ちる。そうなると拒否権が正しい経路まで
        /// 棄却しはじめるため、痩せすぎる手前で見送る。
        /// </summary>
        ...
        /// 前処理の相互訂正より厳しくしているのは、こちらは重ねた結果を
        ...
        /// </summary>
        public const double ペア結合の許容不一致率 = 0.05;
    }
}
