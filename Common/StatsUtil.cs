namespace Tsumiki.Common
{
    ...
    /// 分布の要約統計をまとめる。「整列してから累積和が半分を超えた点を採る」
    /// という同じ骨格の実装がファイルごとに個別に書かれていたのを1箇所にする。
    /// </summary>
    internal static class StatsUtil
    {
        ...
        /// <summary>
        /// 長さで重み付けした値の中央値。累積長が総延長の半分を超えた点の
        /// 値を採る。短い断片が本数で多数を占めていても、実際の塩基の
        ...
        /// <summary>
        /// 長さ一覧から N50/L50 を求める。N50 は「この長さ以上の配列だけで
