...
namespace Tsumiki.Commons
{
    ...
    internal class Util
    {
        ...
        public static long V_変換_メモリサイズ(string p_表記)
        {
            ...
            if (!double.TryParse(l_本体, System.Globalization.NumberStyles.Float, System.Globalization.CultureInfo.InvariantCulture, out var l_数値) || l_数値 <= 0D)
            {
                throw new ArgumentException($"Could not read '{p_表記}' as a memory size (e.g. 2G, 512M, 1024)");
            }

            var l_バイト数 = (long)(l_数値 * l_倍率);
            return l_バイト数 <= 0L ? throw new ArgumentException($"Memory size '{p_表記}' is too small") : l_バイト数;
        }
        ...
        private static int Get_最小回転開始位置(string p_配列)
        {
            ...
            {
                var l_文字 = l_二重化[j];
                var l_i = l_失敗関数[j - l_k長 - 1];
                while (l_i != -1 && l_文字 != l_二重化[l_k長 + l_i + 1])
                {
