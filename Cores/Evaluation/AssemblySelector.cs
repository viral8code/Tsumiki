...
namespace Tsumiki.Cores.Evaluation
{
    ...
    /// 連続性の利得が完全性の損失を上回りうるため、配列を大きく落とした誤アセンブリのほうが高い点になる<br/>
    /// 順序を明示した二段階にしてある<br/>
    /// 完全長細菌ゲノムを目標にする場合、NG50 だけを見る比較は「4.5 Mb の染色体が 2 本に割れ、5 kb のプラスミドが完全に閉じたアセンブリ」と「4.5 Mb が 1 本に閉じプラスミドを取りこぼしたアセンブリ」を正しく順序づけられない<br/>
    /// 完全性・正確性で足切りしたあとの候補群を、「閉じた複製単位の本数 → その総塩基がゲノム推定サイズに占める割合 →完全性 → 正確性 → NG50」の順で比べる<br/>
    /// 完全性と正確性は、推定の揺らぎに収まる差を同点として扱う<br/>
    /// ここを生の値で比べると、足切りで「意味がない」と宣言したのと同じ幅の差がそのまま順位を決めてしまう<br/>
    /// この 2 つは k に対して逆向きに動く<br/>
    /// k を上げるほどグラフが解けて完全性は上がり、反復を正しく複製したぶん「出しすぎ」が増えて正確性は下がる<br/>
    /// 逆相関している以上、生の値で順に見れば「先に見たほうの端の k」が機械的に選ばれるだけで、比較になっていない<br/>
    /// 揺らぎの範囲を同点にすることで、実質的な差があるときだけこの 2 つが効き、無いときは連続性で決まるようにする<br/>
    /// 2 つのうち完全性を先に見る<br/>
    /// 取りこぼしはゲノムをそのまま失うので多いほど確実に悪い<br/>
    /// 一方「出しすぎ」は、反復を正しく複製したときにも期待コピー数の丸め次第で増える向きに動くため、指標として反復解決に不利な偏りを持つ<br/>
    /// 先に見るなら偏りの無いほうにする<br/>
    /// 本当に危険な重複 (統合が誤結合を持ち込んだ場合など) は正確性の足切りが捕まえる
    /// </remarks>
    internal static class AssemblySelector
    {
        ...
        /// </summary>
        /// <remarks>
        /// 統合で誤結合を持ち込んだ場合はこれを大きく超えて落ちる (実データでは 13 ポイント落ちた例がある)
        /// </remarks>
