...
namespace Tsumiki.Tests.Utility
{
    ...
    public class KmerLengthSelectorTests
    {
        ...
        {
            Assert.Equal(p_期待値, KmerLengthSelector.Get_推奨k長(p_リード長));
        }
        ...
        public void V_推奨k長は常に奇数でリード長より短い()
        {
            for (var l_リード長 = Consts.自動k長に必要な最小リード長; l_リード長 <= 400; l_リード長++)
            {
                ...
                Assert.True(l_suggestion.Value % 2 == 1, $"k={l_suggestion} for read length {l_リード長} is even");
                Assert.True(l_suggestion.Value < l_リード長, $"k={l_suggestion} is not shorter than read length {l_リード長}");
