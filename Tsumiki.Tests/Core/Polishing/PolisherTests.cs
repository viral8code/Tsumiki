...
namespace Tsumiki.Tests.Core
{
    ...
    /// </summary>
    /// <remarks>
    /// 直すべきものを直すことと同じくらい、根拠が無い位置を動かさないことが重要なので、両方を確かめる
    /// </remarks>
    public class PolisherTests : IDisposable
    {
        ...
        {
            this._一時ディレクトリ = Path.Combine(Path.GetTempPath(), "tsumiki_polish_" + Guid.NewGuid().ToString("N"));
        ...
        public void Get_磨いた結果_リードが支持する塩基へ置換を直す()
        {
            ...
            Assert.Equal(1L, l_統計!.Value.A_訂正した塩基数);
        ...
        /// <returns>書き出したパス</returns>
        private string V_書き出し_FASTQ(string p_真の配列, int p_リード長, int p_刻み, int p_開始 = 0, int? p_終了 = null)
        {
