...
namespace Tsumiki.Tests.Core
{
    ...
    /// </summary>
    /// <remarks>
    /// リード 1 本に収まる配列はリードそのもの以上の情報を運ばず、この帯には繋ぎ間違えた断片が集まりやすい
    /// </remarks>
    public class ShortSequenceFilterTests : IDisposable
    {
        ...
        {
            this._作業ディレクトリ = Path.Combine(Path.GetTempPath(), "tsumiki_shortfilter_tests_" + Guid.NewGuid().ToString("N"));
        ...
        {
            var l_パス = this.Get_書き出し(("SCAFFOLD1", new string('A', 300)), ("SCAFFOLD2", new string('C', 150)), ("SCAFFOLD3", new string('G', 149)), ("SCAFFOLD4", new string('T', 1)));
        ...
        {
            var l_パス = this.Get_書き出し(("SCAFFOLD1", new string('A', 300)), ("SCAFFOLD2", new string('C', 200)));
        ...
        public void V_削除_中間ファイル_最終成果物とログだけ残す()
        {
            ...
            _ = Directory.CreateDirectory(Path.Combine(this._作業ディレクトリ, "k93"));
            File.WriteAllText(Path.Combine(this._作業ディレクトリ, "k93", "unitigs.fasta"), "x");

            Tsumiki.Cores.Pipeline.AssemblyWorkspace.V_削除_中間ファイル(this._作業ディレクトリ);

            var l_残り = Directory.EnumerateFiles(this._作業ディレクトリ)
                .Select(Path.GetFileName).Order(StringComparer.Ordinal).ToList();
            Assert.Equal([ "Tsumiki.log", "assembly.ambiguous.tsv", "assembly.gfa", "assembly.report.json", "contigs.fasta", "scaffolds.fasta", "unitigs.fasta", ], l_残り);
        ...
        {
            var l_パス = Path.Combine(this._作業ディレクトリ, "scaffolds.fasta");
